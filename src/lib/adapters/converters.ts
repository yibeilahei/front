 ...
 * Auto is Japanese-first (縦書き CSS / 電書協 / kana).
 */

import { encodeXthPage, buildXtchContainer, outputNameFromSource } from "../xtch";
import { createEncodePool, type XtchEncodePool } from "../xtchEncodePool";
...
async function convertSession(
  ...
  const pages: Uint8Array[] = new Array(limit);
  // Rendering (`session.pager.renderPage`) drives an iframe/canvas, so it
  // must stay on the main thread — but encoding (quantize + raw-DEFLATE) is
  // pure data work. Pipelining it onto a worker pool lets page i+1 start
  // rendering while page i is still being encoded/compressed, instead of
  // blocking the main thread on both steps serially for every page.
  const pool: XtchEncodePool | null = createEncodePool();
  const inFlight: Promise<void>[] = [];
  const maxInFlight = pool ? pool.size + 1 : 1;
  try {
