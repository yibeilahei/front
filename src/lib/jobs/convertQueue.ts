...
/**
 * Extrapolates remaining time from elapsed time and overall completion
 * fraction (linear ETA). Suppressed until a small amount of progress/time
 * has passed, since early samples swing wildly (e.g. page 1 of a 300-page
 * book taking 200ms would "predict" a 1-minute book).
 */
function estimateRemainingMs(elapsedMs: number, overallFrac: number): number | null {
  if (overallFrac <= 0.03 || overallFrac >= 1 || elapsedMs < 800) return null;
  const remaining = (elapsedMs * (1 - overallFrac)) / overallFrac;
  return remaining > 500 ? remaining : null;
}
...
export async function runConvertQueue(
  ...
  let cancelled = false;
  // Wall-clock start of *this* queue run, used to extrapolate an ETA from
  // "overall fraction done" (same job-equal-weight approximation the
  // existing `(i + p) / pending.length` progress % already uses).
